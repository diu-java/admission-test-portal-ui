...
@Component({
  selector: 'app-admission-report-admit-card',
  templateUrl: './admission-report-admit-card.component.html',
  styleUrls: ['./admission-report-admit-card.component.css']
})
export class AdmissionReportAdmitCardComponent implements OnInit{
  search:any='';
  paymentStatus:any=1;
  ...
  ngOnInit() {
    ...
    const searchAdmissionReports = this.admissionReportService.getSearchAdmissionReportAdmitCard();
    if(searchAdmissionReports){
      const {semesterCode, programCode, intakeCode} = searchAdmissionReports;
      this.semesterCode = semesterCode  || '';
      this.programCode = programCode  || '';
      this.intakeCode = intakeCode  || '';
      this.loading = true;
      this.admissionReportService.getAdmissionReportAdmitCard(semesterCode, programCode,intakeCode).subscribe((response:any)=>{
        this.loading = false;
        if(response.status){
  ...
  exportToExcel(): void {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Admission Report-Admit Card');
    XLSX.writeFile(wb, 'Admission Report - Admit Card '+formattedDate+'.xlsx');
  }
  getAdmissionReport() {
    this.loading = true;
    this.admission_reports = [];
    const searchAdmissionAReport = {semesterCode: this.semesterCode, programCode: this.programCode, intakeCode: this.intakeCode};
    this.admissionReportService.setSearchAdmissionReportAdmitCard(searchAdmissionAReport)
    if(this.semesterCode){
      this.admissionReportService.getAdmissionReportAdmitCard(searchAdmissionAReport.semesterCode, searchAdmissionAReport.programCode, searchAdmissionAReport.intakeCode).subscribe((response:any)=>{
        this.loading = false;
        if(response.status){
          this.admission_reports = response.data;
        }else {
