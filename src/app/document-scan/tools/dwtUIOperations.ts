...
import type { ThumbnailViewer, ThumbnailViewerSettings, ViewMode } from "dwt/dist/types/WebTwain.Viewer";
import type { WebTwain } from "dwt/dist/types/WebTwain";

import {
  getEl, getInputEl, getSelectEl,
  elAddClass, elRemoveClass, elHide,
  isNumber, isString, browserInfo } from "./common";
...
export class DwtUIOperations {
  ...
  onPageInit() {
    ...
    let _this = this;
    this.dwtService.init(function (dwtObject: WebTwain) {
      ...
      typeof error === "string" ? checkErrorStringWithErrorCode(-1, error) : addMessage(error);
    });
  ...
  updateViewer() {
    ...
    if (this.dwtObject) {
      ...
      this.thumbnail = this.dwtObject?.Viewer.createThumbnailViewer(<ThumbnailViewerSettings>{ size: "25%" });
  ...
  getBlob(strExtension: string, strAllPages: string): Promise<any>{
    return new Promise((res, rej) => {
      ...
      this.dwtService.getBlob(selectedIndices, enumImageType).then(
        (data) => {
          res(data);
        }
      )
  ...
  getData(strExtension: string, strAllPages: string): Promise<any>{
    return new Promise((res, rej) => {
      if (!this.dwtObject || !this.checkIfImagesInBuffer()) {
        return;
      }
      ...
      this.dwtService.getBase64(selectedIndices, enumImageType).then(
        (data: any) => {
          res(data);
        }
      )
  ...
  save(saveType: string, strFileNameWithoutExt: string, strExtension: string, strAllPages: string, resolution?: number) {
    ...
    if (bUpload) {
      ...
      //
      // let _this = this;
      // this.dwtService.uploadToServer(selectedIndices,
      //   enumImageType,
      ...
      //   },
      //   error => typeof error === "string" ? checkErrorStringWithErrorCode(-1, error) : addMessage(error)
      // );
      ...
      let _this = this;
      this.dwtService.saveLocally(selectedIndices, enumImageType, strFileNameWithoutExt, true)
        .then(() => {
          checkErrorStringWithErrorCode(0, "Successful.");
        },
        error => typeof error === "string" ? checkErrorStringWithErrorCode(-1, error) : addMessage(error)
  ...
  private _initedDynamicWebTWAIN(dynamicWebTWAINInstance: WebTwain, bNoDevices: boolean) {
    ...
    if (dynamicWebTWAINInstance) {
      if (dynamicWebTWAINInstance.ErrorCode == 0) {
        let thumbnailViewer: any = dynamicWebTWAINInstance.Viewer.createThumbnailViewer();
        if (thumbnailViewer) {
        ...
        if (twainsource) {
          ...
          dynamicWebTWAINInstance.RegisterEvent("OnPostLoad", (path?: any, name?: any, type?: any) => {
...
function checkErrorStringWithErrorCode(errorCode: number, errorString: string, responseString?: string) {
  if (errorCode == 0) {
  ...
  if (errorCode == -2115) //Cancel file dialog
    ...
    if (errorCode == -2003) {
      if (responseString && responseString != "") {
        let ErrorMessageWin: any = window.open("", "ErrorMessage", "height=500,width=750,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no");
...
function handleOnGetFilePath(bSave: any, count: any, index: any, path: any, name: any) {

}

function appendUploadedItem(strImageName: string, strCustomInfo: string, removeUrl: string, downloadUrl: string) {
...
function initStyle() {
  ...
  let newCssStyle: string[] = [], screenWidth = screen.width, screenHeight = screen.height, bIE = browserInfo.bIE;
