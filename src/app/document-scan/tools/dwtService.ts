...
import {dynamsoftEnvironment} from "../../../environment/dynamsoftEnvironment";
...
export class DwtService {
  ...
  init(_serviceInitReadyCallback: Function, _serviceInitErrorCallback: Function) {

    // ResourcesPath & ProductKey must be set in order to use the library!
    Dynamsoft.DWT.ResourcesPath = dynamsoftEnvironment.Dynamsoft.resourcesPath;
    Dynamsoft.DWT.ProductKey = dynamsoftEnvironment.Dynamsoft.dwtProductKey;
    Dynamsoft.DWT.ServiceInstallerLocation = dynamsoftEnvironment.Dynamsoft.serviceInstallerLocation;
   ...
   * @param fileName Specify the file name.
   */
  uploadToServer(indices: number[], type: number, fileName: string, customInfo: string): Promise<any> {
    return new Promise((res, rej) => {
      ...
      let url = dynamsoftEnvironment.Dynamsoft.uploadTargetURL;
