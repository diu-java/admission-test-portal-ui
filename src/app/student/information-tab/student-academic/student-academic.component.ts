...
export class StudentAcademicComponent implements OnInit{
  ...
  postAcademic() {
    this.studentInformation.studentPersonId = this.personInformation.id;
  ...
  putAcademic() {
    ...
    this.studentInformationService.putStudentInformation(this.studentInformation, this.studentInformation.id).subscribe((response:any)=>{
      if (response.status){
        ...
        this.form?.resetForm(this.studentInformation);
        this.toastr.success(response.message);
        let indexToUpdate = this.student_informations.findIndex((item: AddressInformation) => item.id === this.studentInformation.id);
